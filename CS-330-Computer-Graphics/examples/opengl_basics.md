# CS-330 OpenGL Basics

## 🎯 Purpose
Demonstrate OpenGL programming fundamentals, shaders, and modern graphics pipeline.
...
// Create VAO and VBO for a triangle
...
unsigned int loadTexture(const char* path) {
    ...
    // Load image data (simplified - in practice use stb_image or similar)
...
// Setup textured rectangle
void setupTexturedRectangle(unsigned int* VAO, unsigned int* VBO, unsigned int* EBO) {
    float vertices[] = {
...
## 🔍 OpenGL Concepts
- **Graphics Pipeline**: Vertex processing, rasterization, fragment processing
- **Shaders**: Vertex and fragment shaders for custom rendering
- **Buffers**: VAO, VBO, EBO for data management
- **Textures**: Image mapping and texture coordinates
- **Lighting**: Phong shading model with ambient, diffuse, specular

## 💡 Learning Points
- OpenGL uses a programmable graphics pipeline
- Shaders run on the GPU for high performance
- Vertex attributes define how data is passed to shaders
- Matrices transform 3D coordinates to 2D screen space
